...
document.addEventListener("DOMContentLoaded", function () {
  ...
  function updateImageState() {
    ...
    // 이전, 다음 이미지의 인덱스 계산
    const prevIndex2 =
      (currentIndex2 - 1 + currentImageList.length) % currentImageList.length;
    ...
    accountElement2.textContent =
      currentDescriptionList[currentIndex2].account2;
  ...
  leftButton.addEventListener("click", function () {
    currentIndex2 =
      (currentIndex2 - 1 + imageLists[selectedYear].length) %
      imageLists[selectedYear].length;
    updateImageState();
  });
});
