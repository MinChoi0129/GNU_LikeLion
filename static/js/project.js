...
document.addEventListener("DOMContentLoaded", function () {
  const imgList = [
    {
      src: "/static/image/exImg1.png",
      description: "더 나은 건강, 더 행복한 삶",
      name: "RE : BORN",
    },
    {
      src: "/static/image/exImg2.png",
      description: "필요한 약을 클릭 한번으로",
      name: "PHAMPHAM",
    },
    {
      src: "/static/image/exImg3.png",
      description: "도대체 가능한 날이 언제야",
      name: "이때 어때",
    },
    {
      src: "/static/image/exImg1.png",
  ...
  function showImage(index) {
    ...
    const currentItem = imgList[index];
    explanationElement.innerHTML = `
      <div class="account">${currentItem.description}
        <div class="name">${currentItem.name}</div>
        <div class="view">VIEW MORE 
        <img class="more" src="/static/image/more.png" />
        </div>    
      </div>
    `;
